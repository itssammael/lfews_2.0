...
class SsoClientController extends Controller
{
    ...
    public function redirect(Request $request): RedirectResponse
    {
        ...
        $clientId = config('services.login_portal.client_id') ?: 'lfews_client_id';
    ...
    public function callback(Request $request): RedirectResponse
    {
        ...
        $baseUrl = config('services.login_portal.url') ?: 'http://127.0.0.1:8000';
        $clientId = config('services.login_portal.client_id') ?: 'lfews_client_id';
        $clientSecret = config('services.login_portal.client_secret') ?: 'lfews_client_secret';
        $redirectUri = config('services.login_portal.redirect_uri') ?: route('sso.callback');
    ...
    protected function resolveLocalUser(array $ssoUser): User
    {
        ...
        // Attach default non-admin role if roles table exists
        $userRole = Role::where('name', 'User')->orWhere('name', 'Viewer')->first();
        if ($userRole) {
