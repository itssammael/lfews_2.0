...
const isEditing = computed(() => !!props.contour);

watch(() => props.show, (newVal) => {
    if (newVal) {
        if (props.contour) {
            form.name = props.contour.name;
            form.properties = JSON.stringify(props.contour.properties, null, 2);
            form.geometry = JSON.stringify(props.contour.geometry, null, 2);
        } else {
            form.reset();
            form.properties = '{}';
            form.geometry = '{}';
...
const submit = () => {
    try {
        ...
        if (isEditing.value) {
            form.transform(transformData).put(route('hazard-map.update', props.contour.id), {
                onSuccess: () => emit('close'),
            });
        } else {
            form.transform(transformData).post(route('hazard-map.store'), {
                onSuccess: () => emit('close'),
            });
        }
    } catch (e) {
    ...
    <Modal :show="show" @close="close">
        <div class="p-6">
            <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                {{ isEditing ? 'Edit Contour' : 'Add New Contour' }}
            </h2>

            <div class="mt-6">
                <div class="mb-4">
                    <InputLabel for="name" value="Contour Name" />
                    <TextInput
                        id="name"
                        v-model="form.name"
                        type="text"
            ...
            <div class="mt-6 flex justify-end">
                ...
                <PrimaryButton
                    ...
                    @click="submit"
                >
                    {{ isEditing ? 'Update Contour' : 'Create Contour' }}
                </PrimaryButton>
            </div>
        </div>
    </Modal>
