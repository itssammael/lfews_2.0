...
import TextArea from '@/Components/TextArea.vue'; // Correctly imported now
...
const form = useForm({
    ...
    properties: JSON.stringify(props.river.properties, null, 2),
    geometry: JSON.stringify(props.river.geometry, null, 2),
});
...
const submit = () => {
    try {
        ...
        })).put(route('rivers.update', props.river.id));
    } catch (e) {
...
<template>
    <AppLayout title="Edit River">
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
                Edit River: {{ river.name }}
            </h2>
        </template>
